...
# FIX: REMOTE_IP must be defined before it is referenced
REMOTE_IP="192.168.100.36"
log "iperf3: Remote Server (${REMOTE_IP})..."
if ping -c 1 -W 2 "$REMOTE_IP" > /dev/null 2>&1; then
    $PIN iperf3 -c "$REMOTE_IP" -p 5201 -t 10 \
        --json --get-server-output \
        > "${RAW_DIR}/iperf3_remote.json" 2>&1 \
        || echo "iperf3 remote failed" > "${RAW_DIR}/remote_err.log"
else
...
# FIX: drop_caches is read-only in containers — suppress gracefully with || true
# FIX: missing > redirection operator on both dd lines
# FIX: rm -f was incorrectly joined onto the last dd line — separated onto its own line
