...
run_fio() {
    ...
    log "fio: $name..."
    # Corrected: Ensure $extra is treated as an argument to fio, not its own command
    $PIN  fio --name="$name" --filename="$TEST_FILE" --size=1G --bs="$bs" --rw="$rw" \
...
$PIN dd if=/dev/zero of="$TEST_FILE" \
    bs=1M count=1024 \
    oflag=direct \
    status=progress \
    > "${RAW_DIR}/dd_write.txt" 2>&1

$PIN dd if="$TEST_FILE" of=/dev/null \
    bs=1M \
    iflag=direct \
    status=progress \
    > "${RAW_DIR}/dd_read.txt" 2>&1
